...
> **Status: analysis and design only — no caching is implemented in the current codebase.** This document describes the opportunity and the recommended design.
...
At capstone scale (single backend instance, MySQL localhost) Caffeine is the right call. Redis is recorded in [`architecture/05-deployment-architecture.md`](architecture/05-deployment-architecture.md) and `decision-records.md` as the future multi-instance step.
...
- [`logging-strategy.md`](logging-strategy.md) — how cache hits/misses would be observable
- [`decision-records.md`](decision-records.md) — ADR: "No Redis at capstone scale"
