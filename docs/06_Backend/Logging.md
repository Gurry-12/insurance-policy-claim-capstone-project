# Logging — Current State & Observability Strategy

> Consolidates the legacy `docs/logging.md` (current state) and
> `docs/logging-strategy.md` (target design) into one authoritative doc.
> Canonical home: `06_Backend/`.
...
4. **`DEBUG` + `show-sql` in the base config** — noisy in any shared/runtime environment; belongs to a dev profile only (see Part 2 §3).
...
## Part 2 — Target observability strategy (not yet implemented)
...
4. Keep dev noise (SQL, DEBUG) out of the default configuration.
...
- [`../01_System_Architecture/Security_Architecture.md`](../01_System_Architecture/Security_Architecture.md) — audit logging (`SECURITY_AUDIT` logger) vs operational logging
- [`Performance.md`](Performance.md) — related hardening
- [`../11_Developer_Guide/Deployment.md`](../11_Developer_Guide/Deployment.md) — ELK topology
- [`../07_Design_Patterns/Decision_Records.md`](../07_Design_Patterns/Decision_Records.md) — ADR-012 (DB tables as audit source of truth)
