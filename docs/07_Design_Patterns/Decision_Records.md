...
Short records of the notable engineering decisions in this project. **Status:** `Accepted` = implemented today; `Proposed` = recommended (documented but not implemented). Relocated from the legacy flat `docs/` tree; the canonical home is now `07_Design_Patterns/`.
...
- **Consequences:** Shared docs (`docs/`, `demo-data/`, `screenshots/`) live at the root; each module keeps its own README/build. CI would need to build two artifacts.
...
- **Status:** Accepted (superseded in part by the refresh-token architecture — see [`../01_System_Architecture/Security_Architecture.md`](../01_System_Architecture/Security_Architecture.md))
...
- **Consequences:** Simple, horizontally scalable auth. Trade-offs: no server-side revocation, token lives until expiry, `localStorage` exposure risk. The shipped system mitigates this with rotating refresh tokens in an HttpOnly cookie, `tokenVersion` claims, and reuse detection (see Security_Architecture).
...
- **Decision:** Gmail SMTP (`EmailService`) + Twilio (`SmsService`) send 6-digit OTPs; `OtpVerification` enforces expiry (5 min), max 5 attempts, 60s resend window. OTP is also sent for forgot-password/reset.
...
- **Consequences:** Adding a new premium type = new calculator bean; math is unit-testable in isolation. `RoundingRule` enum exists to support rounding policies. See [`Strategy.md`](Strategy.md).
...
- **Status:** Proposed (design in [`../06_Backend/Caching.md`](../06_Backend/Caching.md))
...
- **Status:** Proposed (design in [`../06_Backend/Performance.md`](../06_Backend/Performance.md))
...
- **Consequences:** Fast to iterate; no versioned migrations, so schema drift across environments is possible. **Proposed:** adopt Flyway/Liquibase for production (see [`../04_Database/Table_Descriptions.md`](../04_Database/Table_Descriptions.md)).
...
- **Consequences:** Cheap storage in DB; depends on Cloudinary credentials; upload is synchronous (async path proposed in [`../06_Backend/Performance.md`](../06_Backend/Performance.md)).
...
- **Consequences:** Audit is queryable in-app (timeline UI). **Proposed:** mirror key events to structured logs for ops (see [`../06_Backend/Logging.md`](../06_Backend/Logging.md)).
...
- **Consequences:** Enables load-balancer health checks and uptime visibility; requires careful exposure config (see [`../01_System_Architecture/Security_Architecture.md`](../01_System_Architecture/Security_Architecture.md)).
...
See also: [`../01_System_Architecture/Security_Architecture.md`](../01_System_Architecture/Security_Architecture.md), [`../06_Backend/Performance.md`](../06_Backend/Performance.md), [`../06_Backend/Caching.md`](../06_Backend/Caching.md), [`../06_Backend/Logging.md`](../06_Backend/Logging.md).
